...
async def cmd_start(event: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    if not await check_subscription(user_id):
        ...
        kb = sub_check_kb()
        if isinstance(event, types.Message): return await event.answer(text, reply_markup=kb)
...
async def buy_menu(callback: types.CallbackQuery):
    ...
    if len(devices) >= MAX_DEVICES and user_id not in ADMIN_IDS:
        return await callback.message.edit_text(
            f"<b>⚠️ Лимит устройств!</b>\n\nВы использовали лимит ({MAX_DEVICES}).",
    ...
    text = (
        ...
        "✅ Не блокируется (Reality)\n"
        f"⚠️ Лимит: до {MAX_DEVICES} устройств\n\n"
        "💰 <b>Цена: 1 USDT</b>"
    )
    builder = InlineKeyboardBuilder()
...
async def start_pay(callback: types.CallbackQuery, state: FSMContext):
    ...
    if user_id in ADMIN_IDS:
        dev_name = f"Admin_{int(time.time())}"
        # Пытаемся создать клиента в панели
        new_uuid = xui.add_client(user_id, dev_name)
        if new_uuid:
            await db.add_device(user_id, dev_name, new_uuid, expiry_days=365)
            link = os.getenv("VLESS_TEMPLATE").format(uuid=new_uuid, device_name=dev_name)
            await callback.message.answer(f"👑 <b>Админ-доступ активирован:</b>\n\n<code>{link}</code>")
        else:
            await callback.answer("Ошибка связи с 3X-UI. Проверьте логи панели.", show_alert=True)
        return
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📦 Устройство: {message.text}\nНажмите кнопку после оплаты:", reply_markup=builder.as_markup())
...
async def check_p(callback: types.CallbackQuery, state: FSMContext):
    ...
    if invoices and invoices[0].status == 'paid':
        ...
        if u_uuid:
            ...
            await callback.answer("Ошибка генерации ключа в панели.", show_alert=True)
    else:
...
async def process_trial(callback: types.CallbackQuery):
    ...
    if new_uuid:
        await db.add_device(user_id, dev_name, new_uuid, expiry_days=2)
        await db.set_trial_used(user_id)
        link = os.getenv("VLESS_TEMPLATE").format(uuid=new_uuid, device_name=dev_name)
        await callback.message.answer(f"🎁 <b>Тест на 2 дня выдан!</b>\n\n<code>{link}</code>", reply_markup=back_to_main_kb())
    else:
        # Если сюда попали - значит xui.add_client вернул None
        await callback.answer("Ошибка связи с сервером 3X-UI. Проверьте настройки API.", show_alert=True)
...
async def admin_give_res(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        new_uuid = xui.add_client(t_id, "Gift_Key")
        if new_uuid:
            await db.add_device(t_id, "Gift_Key", new_uuid, expiry_days=365)
            await message.answer(f"✅ Выдано на год юзеру {t_id}")
        else:
            await message.answer("Панель не выдала ключ.")
...
async def profile(callback: types.CallbackQuery):
    ...
    text = f"<b>👤 Профиль</b>\nID: <code>{callback.from_user.id}</code>\n\n<b>Ключи ({len(devices)}/{MAX_DEVICES}):</b>\n"
    for d in devices:
        link = os.getenv("VLESS_TEMPLATE").format(uuid=d['uuid'], device_name=d['device_name'])
        text += f"— {d['device_name']}: <code>{link}</code>\n"
    await callback.message.edit_text(text if devices else "Нет ключей", reply_markup=back_to_main_kb())
